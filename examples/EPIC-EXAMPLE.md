...
### UI / Contrato de componentes
Estos tipos son el contrato que todos los componentes de UI deben usar para props y estado. Un componente que renderiza una lista de productos usará `Product[]`. Un formulario de creación usará `NewProduct`. Ningún componente definirá sus propias interfaces para entidades del dominio — siempre importarán desde `@/types/database`.
...
## Estados UI requeridos
No aplica — módulo de tipos sin componentes visuales.
...
## Objetivo
Crear el middleware de Next.js que intercepta cada request, valida el JWT de Supabase, refresca el token si está por expirar, y resuelve el `organization_id` y `role` del usuario autenticado inyectándolos como headers en la request. Es la única puerta de entrada a todas las rutas protegidas. Sin él, las rutas del dashboard y del founder son accesibles sin autenticación. También crea la página de login mínima funcional necesaria para el test de integración del milestone.
...
| `app/(auth)/login/actions.ts` | CREATE | Server Action para autenticación con Supabase Auth |
...
export async function middleware(request: NextRequest) {
  ...
  // 6. Redirect a /login si no autenticado en rutas protegidas
}
